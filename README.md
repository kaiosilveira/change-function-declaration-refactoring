...
In this work example we have a `Book` class that takes customer reservation. We want to add a new functionality, though: a priority list, so customers on the priority list will receive the book first.
...
The test suite for the book class covers the main behaviors of the `addReservation` function. It also covers the behavior of an auxiliar function we've introduced to `fetchReservations`:
...
To perform the steps for this refactoring, we first need to introduce a new `addReservation` function. We can't use the same name, of course, so we picked `zzNEW_addReservation`. Fowler used `zzAddReservation` on his book, but I think the screaming `NEW` word helps to identify our refactoring intents.
...
Note how this new function has an `assert` statement to make sure the new parameter is provided. This helps at development time, so we don't forget any caller behind nor fall into the trap of using the new function with the old signature.
...
And once all the callers are updates, we can remove old `addReservation` fn:
...
The third working example for this refactoring brings a function that validates whether a given customer is resident of New England. The main reason of this refactoring session is to make the `inNewEngland` function more generic and decouple it from the `customer` entity, accepting a `stateCode` as parameter instead of the full `customer` object.
...
The test suite for the `inNewEngland` is pretty straightforward: It contains two test, one to assert that the function returns `true` if the state code belongs to any state code from New England, and to other to assert the inverse:
...
Then, we call `zzNEW_inNewEngland` as the body of current `inNewEngland` function...
...
Finally, we remove `zzNEW` from `inNewEngland` function's name and update its callers (using the simple mechanic with aid of an automated refactoring tool):
